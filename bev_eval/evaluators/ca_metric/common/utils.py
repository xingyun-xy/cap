...
def x_diff(gt_box: EvalBox, pred_box: EvalBox) -> float:
    """
    L2 distance between the box centers (xy only).
    :param gt_box: GT annotation sample.
    :param pred_box: Predicted sample.
    :return: L2 distance.
    """
    return abs(pred_box.translation[0] - gt_box.translation[0])


def y_diff(gt_box: EvalBox, pred_box: EvalBox) -> float:
    """
    L2 distance between the box centers (xy only).
    :param gt_box: GT annotation sample.
    :param pred_box: Predicted sample.
    :return: L2 distance.
    """
    return abs(pred_box.translation[1] - gt_box.translation[1])
...
def scale_iou(sample_annotation: EvalBox, sample_result: EvalBox) -> float:
    ...
    # if not (all(sa_size > 0) and all(sr_size > 0)):
    #     print (sample_annotation.sample_token)
    #     print (sa_size, sr_size)
