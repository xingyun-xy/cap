...
def print_final_metrics(metrics: TrackingMetrics) -> None:
    ...
    for class_name in class_names:
        ...
        for metric_name in metric_names:
            ...
            print_format = (
                "%f" if np.isnan(val) else metric_name_to_print_format(metric_name)
            )
...
def create_motmetrics() -> MetricsHost:
    ...
    mh.register(
        motp_custom, ["num_detections"], formatter="{:.2%}".format, name="motp_custom"
    )
    mh.register(
        num_fragmentations_custom,
        ["obj_frequencies"],
        formatter="{:.2%}".format,
        name="num_fragmentations_custom",
    )
    mh.register(
        faf,
        ["num_false_positives", "num_frames"],
        formatter="{:.2%}".format,
        name="faf",
    )
    mh.register(
    ...
    mh.register(
        longest_gap_duration, ["obj_frequencies"], formatter="{:.2%}".format, name="lgd"
    )
