...
class TestMain(unittest.TestCase):
    ...
    def _mock_submission(
        nusc: NuScenes, split: str, add_errors: bool = False
    ) -> Dict[str, dict]:
        """
        ...
        def random_class(
            category_name: str, _add_errors: bool = False
        ) -> Optional[str]:
        ...
        for sample in tqdm(val_samples, leave=False):
            ...
            for ann_token in sample["anns"]:
                ...
                tracking_name = random_class(
                    ann["category_name"], _add_errors=add_errors
                )
    ...
    def basic_test(
        ...
        nusc_eval = TrackingEval(
            ...
            nusc_dataroot=os.environ["NUSCENES"],
            verbose=False,
        )
    ...
    def test_delta_mock(self, eval_set: str = "mini_val", render_curves: bool = False):
        ...
        metrics = self.basic_test(
            eval_set, add_errors=True, render_curves=render_curves
        )
    ...
    def test_delta_gt(self, eval_set: str = "mini_val", render_curves: bool = False):
        ...
        metrics = self.basic_test(
            eval_set, add_errors=False, render_curves=render_curves
        )
