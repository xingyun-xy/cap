    ...
    for sample_token, data in gts.items():
        ...
        for gt in data:
            ...
            eb = DetectionBox(
                ...
                translation=gt["trans"],
                size=gt["size"],
                rotation=gt["rot"],
                detection_name=gt["name"],
                attribute_name=gt["attr"],
                velocity=gt["vel"],
            )
            gt_boxes.append(eb)
...
class TestTPSimple(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.car3 = {
            "trans": (3, 3, 1),
            "size": (2, 4, 2),
            "rot": Quaternion(axis=(0, 0, 1), angle=0),
            "score": 1.0,
        }
        self.car4 = {
            "trans": (3, 3, 1),
    ...
    def check_tp(
        ...
        metric_data = get_metric_data(
            gts, preds, detection_name, 2.0
        )  # Distance threshold for TP metrics is 2.0
        ...
        self.assertGreaterEqual(
            ...
            msg="Incorrect {} value".format(metric_name),
        )
    ...
    def test_one_img(self):
        ...
        # Then we take average of all segments and normalize it with the recall values we averaged over.
        target_error = ((0 + 0) / 2 + (0 + 0.5) / 2 + (0.5 + 0.33) / 2) / (
            3 * 0.9
        )  # It is a piecewise linear with 3 segments
        self.check_tp(
            {"sample1": [gt1, gt2, gt3]},
            {"sample1": [pred1, pred2, pred4]},
            target_error=target_error,
            metric_name="trans_err",
        )
    ...
    def test_two_imgs(self):
        ...
        # Two GT's and two detections.
        # The target is the average value of the recall vs. Error curve.
        target_error = ((0 + 0) / 2 + (0 + 0.5) / 2) / (
            2 * 0.9
        )  # It is a piecewise linear with 2 segments.
        self.check_tp(
            {"sample1": [gt1], "sample2": [gt2]},
        ...
        self.check_tp(
            {"sample1": [gt1], "sample2": [gt2], "sample3": []},
        ...
        self.check_tp(
            {"sample1": [gt2, gt3], "sample2": [gt3]},
            {"sample1": [pred2], "sample2": [pred4]},
            target_error=1.0,
            metric_name="trans_err",
        )
