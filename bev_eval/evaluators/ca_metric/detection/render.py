...
def visualize_sample(
    ...
    # Get boxes in ego coordinate.
    boxes_gt = gt_boxes[sample_token]
    boxes_est = pred_boxes[sample_token]
    ...
    for box in boxes_est:
        ...
        assert not np.isnan(box.detection_score), "Error: Box score cannot be NaN!"
        if box.detection_score >= conf_th:
...
def dist_pr_curve(
    ...
    fig, (ax, lax) = plt.subplots(
        ncols=2, gridspec_kw={"width_ratios": [4, 1]}, figsize=(7.5, 5)
    )
    ax = setup_axis(
        xlabel="Recall",
        ylabel="Precision",
...
def summary_plot(
    ...
    for ind, detection_name in enumerate(DETECTION_NAMES):
        title1, title2 = (
            ("Recall vs Precision", "Recall vs Error") if ind == 0 else (None, None)
        )
        ...
        class_pr_curve(
            md_list, metrics, detection_name, min_precision, min_recall, ax=ax1
        )
        class_tp_curve(
            md_list, metrics, detection_name, min_recall, dist_th_tp=dist_th_tp, ax=ax2
        )
...
def detailed_results_table_tex(metrics_path: str, output_path: str) -> None:
    ...
    tex += (
        "\\textbf{Class}    &   \\textbf{AP}  &   \\textbf{ATE} &   \\textbf{ASE} & \\textbf{AOE}   & "
        "\\textbf{AVE}   & "
        "\\textbf{AAE}   \\\\ \\hline "
        "\\hline\n"
    )
