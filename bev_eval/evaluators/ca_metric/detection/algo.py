...
def accumulate(
    ...
    for ind in sortind:
        ...
        if match_gt_idx is not None:
            # print(pred_box.sample_token,match_gt_idx)
            gt_box_match = gt_boxes[pred_box.sample_token][match_gt_idx]
            # ƥ�䵽�����GT,Ȼ�����GT��ego���룬ѡ���Ӧ��dist_th,Ȼ���ٽ�һ���ж��Ƿ�match
            center_dist = np.linalg.norm(gt_box_match.ego_translation)
            for k, v in dist_th_dict.items():
                if center_dist > v[0] and center_dist <= v[1]:
                    dist_th = float(k)
            is_match = min_dist < dist_th
        else:
            # print(match_gt_idx,min_dist)
            is_match = False
        ...
        if is_match:
            ...
            match_data["x_err"].append(x_diff(gt_box_match, pred_box))
            match_data["y_err"].append(y_diff(gt_box_match, pred_box))
    ...
    return DetectionMetricData(
        ...
        precision=prec,
        confidence=conf,
        x_err=match_data["x_err"],
        y_err=match_data["y_err"],
