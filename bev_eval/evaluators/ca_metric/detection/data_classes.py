...
from bev_eval.evaluators.ca_metric.common.data_classes import MetricData, EvalBox

# from bev_eval.evaluators.ca_metric.common.utils import center_distance
from bev_eval.evaluators.ca_metric.detection.constants import (
    DETECTION_NAMES,
    ATTRIBUTE_NAMES,
    TP_METRICS,
)
from cap.data.datasets.bevdepth import view_points
...
class DetectionMetricData(MetricData):
    ...
    def __init__(
        ...
        assert len(x_err) == self.nelem
        assert len(y_err) == self.nelem
    ...
    def serialize(self):
        ...
        return {
            ...
            "x_err": self.x_err.tolist(),
            "y_err": self.y_err.tolist(),
    ...
    def deserialize(cls, content: dict):
        ...
        return cls(
            ...
            x_err=np.array(content["x_err"]),
            y_err=np.array(content["y_err"]),
    ...
    def no_predictions(cls):
        ...
        return cls(
            ...
            x_err=np.ones(cls.nelem),
            y_err=np.ones(cls.nelem),
    ...
    def random_md(cls):
        ...
        return cls(
            ...
            x_err=np.random.random(cls.nelem),
            y_err=np.random.random(cls.nelem),
...
class DetectionBox(EvalBox):
    ...
    def deserialize(cls, content: dict):
        """Initialize from serialized content."""
        return cls(
            sample_token=str(content["sample_token"]),
    ...
    def corners(self, wlh_factor: float = 1.0) -> np.ndarray:
        ...
        """
        # w, l, h = self.size * wlh_factor
        w, l, h = [x * wlh_factor for x in self.size]
        ...
        corners = np.dot(Quaternion(self.rotation).rotation_matrix, corners)
