...
__all__ = ["CaBev3dDataset"]

map_name_from_general_to_detection = {
    "vehicle": "car",
    "bus": "bus",
    "truck": "truck",
    "vehicle_else": "construction_vehicle",
    "tricycle": "motorcycle",
    "bicycle": "bicycle",
...
def get_corners(boxes3d):
    ...
    corners3d = torch.tensor(corners3d, dtype=torch.float32)
...
class Quaternion:
    ...
    def __init__(self, *args, **kwargs):
        ...
        if s == 0:
            ...
            if kwargs:
                ...
                if ("scalar" in kwargs) or ("vector" in kwargs):
                ...
                elif (("axis" in kwargs) or ("radians" in kwargs)
                      or ("degrees" in kwargs) or ("angle" in kwargs)):
                    try:
                    ...
                    angle = (kwargs.get("radians")
                             or self.to_radians(kwargs.get("degrees"))
                             or kwargs.get("angle") or 0.0)
    ...
    def __format__(self, formatstr):
        ...
        string = ("{:" + formatstr + "} " + "{:" + formatstr + "}i " + "{:" +
                  formatstr + "}j " + "{:" + formatstr + "}k")
    ...
    def _fast_normalise(self):
        ...
        if not self.is_unit():
            ...
            if abs(1.0 - mag_squared) < 2.107342e-08:
                mag = (
                    1.0 + mag_squared
                ) / 2.0  # More efficient. Pade approximation valid if error is small
            else:
                mag = sqrt(
                    mag_squared
...
class BasePoints(object):
    ...
    def __init__(self, tensor, points_dim=3, attribute_dims=None):
        ...
        if tensor.numel() == 0:
            ...
            tensor = tensor.reshape((0, points_dim)).to(dtype=torch.float32,
                                                        device=device)
        assert (tensor.dim() == 2
                and tensor.size(-1) == points_dim), tensor.size()
    ...
    def new_point(self, data):
        ...
        new_tensor = (self.tensor.new_tensor(data)
                      if not isinstance(data, torch.Tensor) else data.to(
                          self.device))
        original_type = type(self)
        return original_type(
            new_tensor,
...
def rotation_3d_in_axis(points,
    ...
    assert (len(points.shape) == 3 and len(angles.shape) == 1
            and points.shape[0]
            == angles.shape[0]), (f"Incorrect shape of points "
...
class PointCloud(ABC):
    ...
    def __init__(self, points: np.ndarray):
        ...
        assert points.shape[0] == self.nbr_dims(), (
            "Error: Pointcloud points must have format: %d x n" %
            self.nbr_dims())
        self.points = points
...
def map_pointcloud_to_image(
    ...
    # transformed via ego to the image plane.

    lidar_points = LidarPointCloud(lidar_points.T)
    # transform from ego into the camera.
    cam2car_translation = cam2car[:3, -1]
    cam2car_rotation = cam2car[:3, :3]
    lidar_points.translate(-np.array(cam2car_translation))
    # lidar_points.rotate(Quaternion(cam2car_rotation).rotation_matrix.T)
    lidar_points.rotate(cam2car_rotation.T)
...
class Box:
    ...
    def __eq__(self, other):
        ...
        vel = np.allclose(self.velocity, other.velocity) or (np.all(
            np.isnan(self.velocity)) and np.all(np.isnan(other.velocity)))
...
class CaBev3dDataset(data.Dataset):
    ...
    def __init__(self,
        ...
        for json_path in self.json_path_list:
            with open(json_path, 'r') as f:
                sample_list = mmcv.load(f, file_format='json')["sample_list"]
                for sample in sample_list:
                    self.infos.append(sample)
            f.close()
        # print (len(self.infos))
        # self.infos 里面存储所有annotation信息，每一个info代表一个sample
    ...
    def _get_sample_indices(self):
        ...
        for idx, info in enumerate(self.infos):
            gt_names = set(
                [ann_info['type'] for ann_info in info['sample_annotation']])
    ...
    def sample_ida_augmentation(self, ratio):
        """Generate ida augmentation values based on ida_config."""
        H, W = self.ida_aug_conf['H'], self.ida_aug_conf['W']
        fH, fW = self.ida_aug_conf['final_dim']
        if self.is_train:
            resize = np.random.uniform(*self.ida_aug_conf['resize_lim']) # （0.35.0.37）
            resize_dims = (int(W * resize), int(H * resize)) # h--> (315, 333)
            ...
            resize = max(fH / H, fW / W) # (320 / 900, 576 / 1600) = (0.356, 0.36)
            # resize = (704. / 1600., 256. / 900.)
            resize_dims = (int(W * resize), int(H * resize)) #(324, 576)
            newW, newH = resize_dims # (324,576)
            crop_h = newH - fH
            ...
            flip = False
            rotate_ida = 0
        # print ('resize_dims', resize_dims, 'crop', crop)
        return resize*ratio, resize_dims, crop, flip, rotate_ida
    ...
    def get_lidar_depth(self, lidar_points, img, cam_info):
        cam2car = np.array(cam_info['cam2car'])
        cam_instrins = np.array(cam_info['intrinsic'])

        pts_img, depth = map_pointcloud_to_image(lidar_points.copy(), img,
                                                 cam2car.copy(),
                                                 cam_instrins.copy())
    ...
    def get_image(self, cam_infos, cams, lidar_infos=None):
        ...
        for cam in cams:
            ...
            w, h = key_info[cam]["size"]
            ratio = max(900 / h, 1600 / w)
            resize, resize_dims, crop, flip, \
                rotate_ida = self.sample_ida_augmentation(
                    ratio)
            # print (resize, resize_dims)
            for sweep_idx, cam_info in enumerate(
                ...
                img = Image.open(
                    os.path.join(self.data_root,
                                 cam_info[cam]['path']))  # 拿到camera的图像文件位置

                if self.visual_imgs:  ########## 如果要进行可视化，这里会把图像装进来
                    self.ori_img.append(
                        np.array(img)[:, :, ::-1].astype(np.uint8))

                sweepsensor2sweepego = torch.tensor(np.array(
                    cam_info[cam]['cam2car']),
                                                    dtype=torch.float32)

                keysensor2sweepsensor = torch.tensor(
                    np.eye(4), dtype=torch.float32)  # key到sweep， 全是当前帧，所以两者一样
                ...
                intrin_mat[3, 3] = 1
                intrin_mat[:3, :3] = torch.Tensor(cam_info[cam]['intrinsic'])
                if self.return_depth and (self.use_fusion or sweep_idx == 0):
                    point_depth = self.get_lidar_depth(
                        sweep_lidar_points[sweep_idx], img, cam_info[cam])
                ...
                if self.is_train:
                    ...
                    if np.random.random() < 0.5:
                        ...
                        hsv_value1 = np.random.random()
                        hsv_value2 = np.random.random()
                        hsv_value3 = np.random.random()
                        #注意输入通道RGB 还是BGR
                        augment_hsv(img,
                                    hgain=hsv_value1,
                                    sgain=hsv_value2,
                    ...
                    gm = GridMask(True,
                                  ...
                                  prob=0.2)
                    img = gm.forward(img)
                img = np.array(img)[:, :, ::-1].copy().astype(
                    np.uint8)  # PIL --> numpy, RGB --> BGR
                # img = mmcv.imnormalize(img, self.img_mean,self.img_std, self.to_rgb)
                if self.visual_imgs:  ########## 如果要进行可视化，这里会把图像装进来
                    # self.post_img.append(mmcv.imdenormalize(img, self.img_mean, self.img_std, to_bgr = False).astype(np.uint8))
                    self.post_img.append(img)

                img = torch.from_numpy(img).permute(2, 0, 1)  # HWC --> CHW
                imgs.append(img)
                intrin_mats.append(intrin_mat)
                timestamps.append(float(cam_info[cam]['timestamp']))
        ...
        if self.is_train:
            ...
            camera_intrinsic = [key_info[cam]['intrinsic']
                                for cam in cams]  # 内参
            file_name = [key_info[cam]['path'] for cam in cams]
            cam_infos = key_info
            img_metas = dict(box_type_3d=LiDARInstance3DBoxes,
        ...
        if self.visual_imgs:  ################ 如果需要可视化，则在这里进行可视化
            ...
            save_path = self.visual_save_path + os.sep + os.path.basename(
                cam_infos[0]["CAM_FRONT"]['path'])  # 以前视camera去命名
    ...
    def get_gt(self, info, cams):
        ...
        gt_boxes = list()
        gt_labels = list()
        for target in info['sample_annotation']:
            box = target['3D']
            if box['visible_in_2d'] == 0: # 过滤掉2d不可见的
                continue
            box_xyz = np.array(box['xyz'])
            box_dxdydz = np.array(
                [box['whl'][2], box['whl'][0], box['whl'][1]])
            box_yaw = np.array([box['yaw']])
            box_velo = np.array(box['velocity'])[:2]
            gt_label = target['type']
            if (map_name_from_general_to_detection[gt_label]
                    not in self.classes):
                continue
    ...
    def bev_gt_vis(
        ...
        for box3d, label in zip(gt_boxes, gt_labels):
            # box3d[4], box3d[3] = box3d[5], box3d[4],box3d[3]
            if np.linalg.norm(box3d[:2]) <= show_range:
                corners = get_corners(box3d[None])[0]
                pred_corners.append(corners)
                pred_class.append(class_map_id[label])
        ...
        for i, k in enumerate(IMG_KEYS):
            ...
            img = mmcv.imread(
                os.path.join(self.data_root, cam_infos[0][k]["path"]))
            ...
            # Draw images
            plt.imshow(img)
            cam2car = np.array(cam_infos[0][k]["cam2car"])
        ...
        # plt.show()
        token = img_metas["token"]
        d_f = os.path.join(self.visual_save_path, f"{token}.png")
        print(d_f)
        plt.savefig(d_f)
    ...
    def __getitem__(self, idx):
        ...
        # clip_list = self.infos['clip_list'
        cam_info = {}
        lidar_info = {}
        for cam in cams:
            cam_info[cam] = info[cam]
            cam_info[cam]["timestamp"] = info["timestamp"]
        cam_infos = [cam_info]
        lidar_info = info["LIDAR_TOP"]
        # cam_infos.append(cam_info)
        lidar_infos = [lidar_info]
        '''
        cam_infos.append(info['CAM_FRONT'])
        cam_infos.append(info['CAM_FRONT_LEFT'])
        cam_infos.append(info['CAM_FRONT_RIGHT'])
        cam_infos.append(info['CAM_BACK'])
        cam_infos.append(info['CAM_BACK_LEFT'])
        cam_infos.append(info['CAM_BACK_RIGHT'])
        cam_infos.append(info['timestamp'])
        cam_infos.append(info['sample_annotation'])
        lidar_infos.append(info['LIDAR_TOP'])
        '''
        ...
        # np.save(str(idx) + 'instrin', sweep_intrins)
        # np.save(str(idx) + 'extral', sweep_sensor2ego_mats)
        # print (torch.tensor([sweep_intrins, sweep_sensor2ego_mats]).shape)

        img_metas["token"] = self.infos[idx]["timestamp"]
        if self.is_train:
        ...
        if self.visual_imgs:
            self.bev_gt_vis(gt_boxes, gt_labels, img_metas, cam_infos)
