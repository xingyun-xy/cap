...
class Config(object):
    ...
    def dump_json(self):
        ...
        return json.dumps(cfg_dict, cls=JSONEncoder, sort_keys=True)
...
def crop_configs(cfg: Config):
    ...
    # {stage}_trainer and {stage}_predictor
    for key in cfg_dict:
        if (
            key in main_keys_list
            or key.endswith("_trainer")
            or key.endswith("_predictor")
            or key.endswith("_solver")
        ):
            output_dict[key] = cfg_dict.get(key, None)
