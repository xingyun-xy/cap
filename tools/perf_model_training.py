...
>> python tools/perf_model_training.py --config tests/data/toy_multitask/multitask.py --step float --iter-nums 100 --allow-all-rank
...
from cap.profiler.model_training_perf import ModelTrainingPerf
...
from trainer_wrapper import TRAINING_STEPS, TrainerWrapper
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=2000,
        type=int,
        help="perf model training iteration nums",
    )
    parser.add_argument(
        "--frequent", default=10, type=int, help="perf model training frequent"
    )
    parser.add_argument(
        "--allow-all-rank",
        action="store_true",
...
def main(
    ...
    with DisableLogger(disable_logger), RegistryContext():
        trainer = TrainerWrapper(
            cfg=cfg, train_step=step, logger=logger, device=device
        )
        trainer.prepare_fit()
        ...
        ModelTrainingPerf(
            trainer._trainer, iter_nums=iter_nums, frequent=frequent
        ).run()
